...
var parseFeed = function(feedXml){
  ...
  var parseRss = function(rss){
    if(rss.channel[0].item === undefined)
      return [];
    return rss.channel[0].item.map(function(item){
      var guid = getElementText(item.guid);
      guid = guid !== undefined ? guid : getElementText(item.link);
      guid = guid !== undefined ? guid : getElementText(item.title);
      guid = guid !== undefined ? guid : new Date().toISOString();
      return {
        guid: guid,
  ...
  if(feedXml['rdf:RDF'] !== undefined && feedXml['rdf:RDF'].$ !== undefined && feedXml['rdf:RDF'].$['xmlns:rdf'] === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#')
    return parseRDF(feedXml['rdf:RDF']);
    ...
    needle.get(url, {parse_response: false}, function(error, response) {
      ...
      parseString(response.body, function(err, result){
        if(err)
          return reject(err);
        try {
          var feedItems = parseFeed(result);
