...
var fetchFeed = function(url){
  ...
  return new Promise(function(resolve, reject){
    needle.get(url, {parse_response: false}, function(error, response) {
      if(error){
        logger.error("Error fetching feed %s", url);
        return reject(error);
      }
      xml2js.parseString(response.body, function(err, result){
        ...
        try {
          ...
          logger.error("Error parsing feed %s", url);
          return reject(err);
        }
      });
    });
...
var cleanup = function(){
  ...
  return persistence.cleanupOrphanedFeeds().then(function(){
    return persistence.cleanupExpiredFeedItems();
  }).catch(logException);
};
