...
var comparePage = function(pageData, oldPage, newContents){
  ...
  pageRecord.contents = newContents;
  var delta = diff.createPatch(pageData.$.url, oldContents+'\n', newContents+'\n');
  if(delta === undefined)
    pageRecord.delta = "";
  else
    pageRecord.delta = decodeURIComponent(delta.replace(/^([^\n]*\n){4}/,''));
  ...
  return new Promise(function(resolve, reject){
    needle.get(page.$.url, {parse_response: false}, function(error, response) {
      if(error){
        ...
        return persistence.findPageMonitorItem(page.$.url, 'url').then(function(oldPage){
  ...
  return persistence.getUserData().then(function(user){
    return new Promise(function(resolve, reject){
      configuration.parseConfig(user.pagemonitor, function(err, pages){
        ...
        var validUrls = pages.pages.page.filter(function(page){
          return page !== undefined && page.$ !== undefined && page.$.url !== undefined;
        }).map(function(page){
          return page.$.url;
        });
        var deletePromises = [];
        return persistence.getPageMonitorItems().then(function(pageMonitorItems){
          return Promise.all(
            pageMonitorItems.filter(function(pageMonitorItem){
              return validUrls.indexOf(pageMonitorItem.url) === -1;
            }).map(function(pageMonitorItem){
              return pageMonitorItem.destroy().catch(logException);
            })
          );
