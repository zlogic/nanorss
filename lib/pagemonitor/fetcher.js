  ...
  return new Promise(function(resolve, reject){
    needle.get(pageMonitorItem.url, {parse_response: false}, function(error, response) {
      if(error){
        ...
        var errorString = 'Error when fetching page: ' + error.message;
        logger.error("Error fetching page %s", pageMonitorItem.url);
        pageMonitorItem.error = errorString;
        return pageMonitorItem.save().then(function(){reject(error);}, reject);
      } else {
...
var update = function(){
  return persistence.getPageMonitorItems().then(function(pageMonitorItems){
    return Promise.all(pageMonitorItems.map(function(pageMonitorItem){
