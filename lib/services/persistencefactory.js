...
var sequelizeConfigurer = function(databaseUrl, sequelizeOptions){
  ...
  if(databaseUrl && sequelizeOptions)
    ...
    sequelize = new Sequelize("sqlite:", {storage: path.resolve(os.tmpdir(), "nanoRSS.sqlite"), isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.SERIALIZABLE, logging: logger.sequelizeLogger});
  ...
  var userPagemonitorConfigurationParsingHook = function(user, options){
    if (!user.changed('pagemonitor'))
      return sequelize.Promise.resolve();
    return user.getPageMonitorItems().then(function(existingItems){
  ...
  var userFeedConfigurationParsingHook = function(user, options){
    if (!user.changed('opml'))
      return sequelize.Promise.resolve();
    return user.getUserFeeds().then(function(existingFeeds){
